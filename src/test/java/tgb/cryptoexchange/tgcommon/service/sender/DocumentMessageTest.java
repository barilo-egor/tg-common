...
class DocumentMessageTest {
    ...
    void shouldCreateAnimationMessageWithPassedParameter(Long chatId, String fileId, String caption, Integer replyToMessageId, ParseMode parseMode) {
        ...
        when(methodExecutor.execute(any(SendDocument.class))).thenReturn(Optional.of(expectedMessage));
        ReplyKeyboard expectedKeyboard = new InlineKeyboardMarkup();
        new DocumentMessage(methodExecutor, chatId, new InputFile(fileId))
                .caption(caption)
                .replyToMessageId(replyToMessageId)
                .parseMode(parseMode)
                .replyKeyboard(expectedKeyboard)
                .send();
        ArgumentCaptor<SendDocument> sendAnimationArgumentCaptor = ArgumentCaptor.forClass(SendDocument.class);
        verify(methodExecutor).execute(sendAnimationArgumentCaptor.capture());
        SendDocument actual = sendAnimationArgumentCaptor.getValue();
        assertAll(
                () -> assertEquals(chatId.toString(), actual.getChatId()),
                () -> assertEquals(fileId, actual.getDocument().getAttachName()),
