...
class PhotoMessageTest {
    ...
    void shouldCreateAnimationMessageWithPassedParameter(Long chatId, String fileId, String caption, Integer replyToMessageId, ParseMode parseMode) {
        ...
        when(methodExecutor.execute(any(SendPhoto.class))).thenReturn(Optional.of(expectedMessage));
        ReplyKeyboard expectedKeyboard = new InlineKeyboardMarkup();
        new PhotoMessage(methodExecutor, chatId, new InputFile(fileId))
                .caption(caption)
                .replyToMessageId(replyToMessageId)
                .parseMode(parseMode)
                .replyKeyboard(expectedKeyboard)
                .send();
        ArgumentCaptor<SendPhoto> sendAnimationArgumentCaptor = ArgumentCaptor.forClass(SendPhoto.class);
        verify(methodExecutor).execute(sendAnimationArgumentCaptor.capture());
        SendPhoto actual = sendAnimationArgumentCaptor.getValue();
        assertAll(
                () -> assertEquals(chatId.toString(), actual.getChatId()),
                () -> assertEquals(fileId, actual.getPhoto().getAttachName()),
