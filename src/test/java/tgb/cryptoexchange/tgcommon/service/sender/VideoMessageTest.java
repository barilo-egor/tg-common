...
class VideoMessageTest {
    ...
    void shouldCreateAnimationMessageWithPassedParameter(Long chatId, String fileId, String caption, Integer replyToMessageId, ParseMode parseMode) {
        ...
        when(methodExecutor.execute(any(SendVideo.class))).thenReturn(Optional.of(expectedMessage));
        ReplyKeyboard expectedKeyboard = new InlineKeyboardMarkup();
        new VideoMessage(methodExecutor, chatId, new InputFile(fileId))
                .caption(caption)
                .replyToMessageId(replyToMessageId)
                .parseMode(parseMode)
                .replyKeyboard(expectedKeyboard)
                .send();
        ArgumentCaptor<SendVideo> sendAnimationArgumentCaptor = ArgumentCaptor.forClass(SendVideo.class);
        verify(methodExecutor).execute(sendAnimationArgumentCaptor.capture());
        SendVideo actual = sendAnimationArgumentCaptor.getValue();
        assertAll(
                () -> assertEquals(chatId.toString(), actual.getChatId()),
                () -> assertEquals(fileId, actual.getVideo().getAttachName()),
